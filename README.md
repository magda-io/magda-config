...
### 2. Install [Terraform](https://learn.hashicorp.com/terraform)
...
(Feel free to use another name other than magda-robot if you like).
...
#### 11. Clone this repo & Initiate Terraform
 and run:
...
As long as you specified `external_domain` in config file [terraform/magda/terraform.tfvars](./terraform/magda/terraform.tfvars) and you've set an `A` record from that domain to the value that came back from `external_ip`, the SSL certificate will be automatically generated and set up for you. The process is going to take 30 to 60 minutes as [specified by Google](https://cloud.google.com/load-balancing/docs/ssl-certificates):
